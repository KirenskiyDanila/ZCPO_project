...
class PhotoController extends AbstractController
{
    ...
    {
        $photo = new Photo();
        $form = $this->createForm(PhotoUploadFormType::class);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $photoFile = $form->get('file')->getData();
            if ($photoFile) {
                $brochureFileName = $fileUploader->upload($photoFile);
                $photo->setName($brochureFileName[0]);
                $photo->setFormat($brochureFileName[1]);
                $post = $doctrine->getRepository(Post::class)->find(1);
                $photo->setPost($post);
                $photo->setPath('img/' . $photo->getName());
    ...
    public function edit(Request $request, Photo $photo, ManagerRegistry $doctrine): Response
    {
